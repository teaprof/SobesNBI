...
class PulpSolver:
    ...
    def buildPulpProblem(self, p: Problem) -> pulp.LpProblem:
        ...
        all_indices = [(i,j) for i in range(p.nProducts) for j in range(p.nProducts)]
        self.x = pulp.LpVariable.dicts("x", all_indices, cat=pulp.const.LpBinary)
        ...
        # where step(x) is Heaviside step function which is implemented bymeans additional constaints:
        #       -(1-u)*M < x <= M*u,
        # where M is some positive constant such that M > sup(abs(x)). These constrains give
        #       if x > 0 then u = 1
        #       if x <= 0 then u = 0  
        ...
        self.u = pulp.LpVariable.dicts("u", all_indices, cat = pulp.const.LpBinary)
        ...
        problem += pulp.lpSum([self.u[i,j]*p.p[j] for i, j in all_indices])    
        return problem
